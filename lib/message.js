   ...
   if(message) {
      thisInstance.onMessages.forEach(function(item) {
         item.call(thisInstance, {
            text: message.messageText?message.messageText.toLowerCase():"",
...
MessageManager.prototype.postMessageToServer = function(msg, onSuccess, onError){
   ...
   if(!thisInstance.token){
      ...
      if(msg){
         ...
         //setTimeout(function(){
            thisInstance.utils.post(thisInstance.apiUrl + '/users/chatMsg/sendMessage', msg, function(error, response) {
              if(error){
                console.log("error calling API sendMessage: ", error)
              }
              if(!response){
                console.log("No response object. FailedAPICall")
                onError && onError({msg: msg, code:'FailedAPICall', message:'Error while calling sendMessage API', error:error});
                
                //STOP
                return
              }
               console.log(response?response.statusCode:"No response object");
