...
BrandchatManager.prototype.onDelivered = function(callback) {
   var thisInstance = this;
   if(callback) {
      thisInstance.onDelivery.push(callback);
   }
};
BrandchatManager.prototype.triggerOnDelivered = function(question) {
   var thisInstance = this;
   if(question) {
   ...
   if(thisInstance.isActive(question.id)){
      if(question) {
         ...
         var questionCleaned = {
              ...
              text: question.messageText.toLowerCase(),
...
BrandchatManager.prototype.joinChat = function(question) {
   var thisInstance = this;
   if(question) {
      thisInstance.messageManager.utils.post(thisInstance.apiUrl + '/users/helpQuestion/joinHelp', {id: question.id}, function(error, response, body){
...
BrandchatManager.prototype.leave = function(questionId) {
   var thisInstance = this;
   if(questionId) {
      thisInstance.messageManager.utils.post(thisInstance.apiUrl + '/users/helpQuestion/leaveHelp', {id: questionId}, function(error, response, body){
         console.log(response.statusCode);
         if (response.statusCode==200) {
...
BrandchatManager.prototype.suggestClose = function(questionId) {
   ...
   if(questionId) {
      setTimeout(function(){
         thisInstance.messageManager.utils.post(thisInstance.apiUrl + '/users/helpQuestion/suggestClosingChat', {id: questionId}, function(error, response, body){
            console.log(response.statusCode);
            if (response.statusCode==200) {
...
BrandchatManager.prototype.forward = function(questionId, filterName, callback) {
   ...
   if(questionId && filterName) {
      setTimeout(function(){
         thisInstance.messageManager.utils.post(thisInstance.apiUrl + '/users/helpQuestion/forwardQuestion', {id: questionId, to:filterName}, function(error, response, body){
            console.log(response.statusCode);
            if (response.statusCode==200) {
...
BrandchatManager.prototype.forceCloseChat = function(question) {
   ...
   if(question) {
      setTimeout(function(){
        thisInstance.messageManager.utils.post(thisInstance.apiUrl + '/users/helpQuestion/forceCloseChat', {id: question.id}, function(error, response, body){
           console.log(response.statusCode);
           if (response.statusCode==200) {
   ...
   if (this.socket) {//Successfully Conected to Chat Server
      ...
      this.socket.on('messageReceived', function(name, message) {
         ...
         switch (message.type) {
            ...
            case 'delivered':
               if(message.item){
                  thisInstance.triggerOnDelivered(message.item);
               }
               break;
