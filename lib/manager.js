...
BrandchatManager.prototype.leave = function(questionId) {
   var thisInstance = this;
   if(questionId) {
      thisInstance.messageManager.utils.post(apiUrl + '/users/helpQuestion/leaveHelp', {id: questionId}, function(error, response, body){
         console.log(response.statusCode);
         if (response.statusCode==200) {
...
BrandchatManager.prototype.connect = function(callback, messageManager) {
   ...
   this.socket = io.connect(chatServerUrl, {
      reconnect : true
   });
